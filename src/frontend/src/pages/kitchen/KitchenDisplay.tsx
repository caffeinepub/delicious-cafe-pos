...
export default function KitchenDisplay() {
  ...
  useEffect(() => {
    if (!pendingInitialized.current) {
      for (const o of pendingOrders) {
        prevPendingIds.current.add(o.id.toString());
      }
    ...
    let newCount = 0;
    for (const o of pendingOrders) {
      if (!prevPendingIds.current.has(o.id.toString())) newCount++;
    }
    if (newCount > 0) {
    ...
    prevPendingIds.current = new Set(pendingOrders.map((o) => o.id.toString()));
  }, [pendingOrders]);
  ...
  function handleMarkComplete(orderId: bigint) {
  ...
  const OrderCard = ({
    ...
    const completingAt = completingOrders.get(key);
    const secondsLeft = completingAt
      ? Math.max(
          0,
          DISMISS_DELAY - Math.floor((Date.now() - completingAt) / 1000),
        )
      : null;
    const progressPct =
      secondsLeft !== null
        ? Math.round((secondsLeft / DISMISS_DELAY) * 100)
        : 100;
    ...
    return (
      <div
        ...
        className={`rounded-xl border p-4 space-y-3 transition-opacity ${
          secondsLeft !== null ? "opacity-60" : "opacity-100"
        } ${
          column === "pending"
  ...
  const totalActive =
    ...
    visibleReadyOrders.length +
    dismissingOrders.length;
    ...
    <div className="min-h-screen flex flex-col" data-ocid="kitchen.section">
      <header className="h-14 flex items-center px-4 border-b border-border bg-card shrink-0">
        ...
        <span className="ml-3 text-sm">
          ...
          {visibleReadyOrders.length + dismissingOrders.length > 0 && (
            <span className="ml-2 text-green-400">
              {visibleReadyOrders.length + dismissingOrders.length} ready
            </span>
          )}
        </span>
        <Button
